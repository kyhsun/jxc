...
<body>
 	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		  ...
		  <tr>
		  	<td>
		  		<form action="<%=basePath %>submitTask.htm" method="POST">
			  		<div align="left" class="STYLE21">
			  			<!-- 任务ID -->
			  			<input type="hidden" name="taskId" value="${taskId }">
			  			<!-- 请假单ID -->
			  			<input type="hidden" name="id" value="${lb.leavebillid}">
				 		请假天数:${lb.day }<br/>
				 		请假原因:${lb.cause }<br/>
				 		<%-- 请假备注:${lb.remark }<br/>--%>
				 		批&emsp;&emsp;注:<textarea rows="5" cols="40" name="comment"></textarea><br/> 
				 		<!-- 使用连线的名称作为按钮 -->
				 		<c:forEach items="${outcomeList }" var="out">
				 				<input type="submit" name="outcome" value="${out}" class="button_ok"/>
				 			</c:forEach>
			  ...
			  <tr>
			    <td><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#a8c7ce" onmouseover="changeto()"  onmouseout="changeback()">
			      ...
			      <c:forEach items="${commentList }" var="com">
			      	<tr>
				        <td height="20" bgcolor="#FFFFFF" class="STYLE6"><div align="center">${com.time }</div></td>
				        <td height="20" bgcolor="#FFFFFF" class="STYLE19"><div align="center">${com.userId }</div></td>
				        <td height="20" bgcolor="#FFFFFF" class="STYLE19"><div align="center">${com.fullMessage }</div></td>
				    </tr> 
			      </c:forEach>
